...
async function loadTitleSceneModules() {
  ...
  titleSceneModulesPromise = (async () => {
...
test('column cap intersections are anchored to object vertical position', async () => {
  const { titleScene } = await loadTitleSceneModules();
  const column = {
    kind: 'column',
    position: [0, 10, 0],
  ...
  const topHit = titleScene.nearestTitleSceneObjectHit([0, 13, 0], [0, -1, 0], [column]);
  const bottomHit = titleScene.nearestTitleSceneObjectHit([0, 7, 0], [0, 1, 0], [column]);

  assert.ok(topHit != null);
  assert.equal(topHit.distance, 2);
  assert.deepEqual(topHit.normal, [0, 1, 0]);
  assert.ok(bottomHit != null);
  assert.equal(bottomHit.distance, 2);
  assert.deepEqual(bottomHit.normal, [0, -1, 0]);
});

test('column side intersections respect object vertical range', async () => {
  const { titleScene } = await loadTitleSceneModules();
  const column = {
    kind: 'column',
    position: [0, 10, 0],
  ...
  const hit = titleScene.nearestTitleSceneObjectHit([2, 10, 0], [-1, 0, 0], [column]);
