# Systems-Style TypeScript Coding Standard
...
Core domain logic must never depend on host-specific APIs, external libraries with side effects, or concrete implementations. All external capabilities are accessed exclusively through **ports** (interfaces). Adapters implement those ports for specific environments.
...
### 3. Encoding / Decoding Only at Boundaries
...
- **P9:** Runtime dispatch (`instanceof`) when inside the same realm; use branding for cross-realm.
