...
const GLOBAL_REGEXP_FLAG = "g";
const BASE64_CHUNK_PATTERN_SOURCE =
  `.{1,${String(BASE64_LINE_WIDTH)}}`;
const GENERATED_LINE_SEPARATOR = "\n";
const GENERATED_CHUNK_SEPARATOR = "";
...
function generatedModule(sourceBytes, packedMask) {
  ...
  const chunks = packedMask
    ...
    .match(
      new RegExp(
        BASE64_CHUNK_PATTERN_SOURCE,
        GLOBAL_REGEXP_FLAG,
      ),
    ) ?? [];
  const chunkLines = chunks
    .map((chunk) => `  "${chunk}",`)
    .join(GENERATED_LINE_SEPARATOR);
