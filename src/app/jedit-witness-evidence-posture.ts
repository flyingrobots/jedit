...
import type { JeditReceiptCorrelation } from '../ports/jedit-receipt-correlation.js';
...
export interface LocalProcessJeditWitnessEvidencePostureInput {
  readonly receiptCorrelation?: JeditReceiptCorrelation['status'];
}

export function localProcessJeditWitnessEvidencePosture(
  input: LocalProcessJeditWitnessEvidencePostureInput = {},
): JeditWitnessEvidencePosture {
  return {
    scope: JEDIT_WITNESS_EVIDENCE_SCOPE_LOCAL_PROCESS,
    receiptCorrelation: input.receiptCorrelation ?? JEDIT_RECEIPT_CORRELATION_MISSING,
