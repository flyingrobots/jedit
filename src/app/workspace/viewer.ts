...
export function renderWorkspace(model: WorkspaceModel): Surface {
  ...
  if (model.columns < MIN_COLUMNS || model.rows < MIN_ROWS) {
    ...
    return renderFeedback(screen, model.notice, model.columns, model.rows);
  }
...
function renderFeedback(screen: Surface, notice: string | null, columns: number, rows: number): Surface {
  return compositeFeedback(screen, notice, columns, rows);
}
