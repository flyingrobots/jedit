...
export function updateTreeFromKey(
  ...
  if (msg.key === 'r') {
    return changeDirectory(model, model.cwd, DIRECTORY_ACTION_REFRESH, deps.fileSystem);
  }
  ...
  if (msg.key === 'backspace' || msg.key === 'left' || msg.key === 'h') {
    ...
    return changeDirectory(model, parent, DIRECTORY_ACTION_OPEN, deps.fileSystem);
  }
  ...
  if (msg.key === 'enter' || msg.key === 'right' || msg.key === 'l') {
    ...
    if (entry.kind === 'dir' || entry.kind === 'parent') {
      return changeDirectory(model, entry.path, DIRECTORY_ACTION_OPEN, deps.fileSystem);
    }
