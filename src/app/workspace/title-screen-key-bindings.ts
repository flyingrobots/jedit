...
function updateTitleRenderKey(
  ...
  if (msg.key === WorkspaceKeys.One) {
    return pushTitleScreenToast({ ...model, titleRenderMode: TITLE_RENDER_MODE.Braille }, TITLE_SHADER_BRAILLE_LABEL, context);
  }
  if (msg.key === WorkspaceKeys.Two) {
    return pushTitleScreenToast({ ...model, titleRenderMode: TITLE_RENDER_MODE.Ascii }, asciiShaderLabel(model), context);
  }
  if (msg.key !== WorkspaceKeys.Period || model.titleRenderMode !== TITLE_RENDER_MODE.Ascii) {
...
function pushAsciiPaletteToast(
  ...
  return pushToast(model, TITLE_ASCII_PALETTE_TOAST_TITLE, titleAsciiPaletteLabel(palette), context);
}
...
function pushToast(
  ...
  }, context.nowMs(), context.createNotificationTickCmd);
}
