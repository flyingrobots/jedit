...
>
> `ADVANCED_GUIDE.md: Fix the "Loading A Buffer" section — loadEditor lives in src/app/workspace/editor-session.ts (signature loadEditor(filePath: string, editorFile: EditorFilePort)), not src/main.ts; src/main.ts routes startup CLI flags and otherwise calls runJeditWorkspace(), which does not read argv. Describe the real load path through loadEditorFile in src/adapters/editor-file.ts, including null-byte binary detection, UTF-8 decoding, and fingerprinting.`
>
