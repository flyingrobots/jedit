...
affordance on overflow, and remains recoverable with Enter or `o` after Escape
dismissal.
...
tokens, Escape can dismiss the selector without trapping the user, Enter or `o`
reopens it from the title screen, and focused workspace specs prove the behavior.
...
- Animating the drawer width through a runtime-owned, critically damped spring
  progress value.
- Adding a recoverable closed-drawer title path: Enter or `o` reopens the
...
in from the left over the frozen title scene. The selector shows the current
...
the list edge. Escape dismisses the selector. From the title screen, Enter or
`o` opens the selector again.

Success is communicated by the visible scrollbar on overflow, selected row
highlighting, and the drawer returning when the user presses Enter or `o` after
Escape. Failure is a long file list with no scroll affordance or a drawer-close
...
- Escape closes an open startup drawer and resets drawer progress to `0`.
- Enter and `o` reopen the startup drawer when no editor is open, the intro is
...
| Reset behavior            | Reopened drawer resets selection to the first row through `openStartupFileModal`; close resets drawer progress to `0`. |
...
stateDiagram-v2
  [*] --> Open
  Open --> ClosedRecoverable: Escape / progress 0
