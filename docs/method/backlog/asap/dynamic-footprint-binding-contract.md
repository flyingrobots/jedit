...
acceptance_criteria:
  ...
  - `ReplaceRangeAsTick` is specified as a bounded hot-text rewrite whose dynamic rope/anchor bindings stay inside a statically declared closure.
...
Define how `jedit`'s real hot-text rewrites should stay dynamically bound at
...
- The stack still wants dishonest rewrite access to fail statically when the
  implementation tries to reach outside its declared hot-text focus.
